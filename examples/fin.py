...
if "openai" not in chat_object_manager.blocked_providers:
    chat_object_manager.blocked_providers.append("openai")
...
def fetch_service(service_id, params, to_display=False):
    try:
        tqdm = None
        should_stop = None
        thread = None
    ...
    def run(
        ...
        **kwargs,
        ...
        try:
                    ...
                    if isinstance(msg.content, List):
                        if len(msg.content) == 2:
                            ...
                            if file_type == "pdf_url":
                                ...
                                file_load_result_dict = fetch_service("file_upload_service", {
            ...
            result_dic = fetch_service("demo_service", params, True)
            ...
            if result is not None:
                if not check(result, True):
                    ...
                    def get(i):
                        global r, finished
                        r_d = fetch_service("demo_service", params, False)
                        rs = r_d.get("output")
                        should_check = result_dic.get("self_check")
                        finished += 1
                        if not should_check:
